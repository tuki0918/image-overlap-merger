...
class ImageOverlapMerger:
    ...
    def find_best_overlap_vertical(
        ...
        if best_overlap > 0:
            ...
            direction_desc = (
                "A bottom + B top" if best_direction == "normal" else "A top + B bottom"
            )
            return (
                best_overlap,
                best_offset,
                f"Vertical merge ({direction_desc}): {best_overlap}px overlap (match rate: {match_ratio:.1f}%)",
                best_direction,
            )
        else:
            return None, 0, "No matching region found for vertical merge", "none"

    def find_best_overlap_horizontal(
        self, img_a: np.ndarray, img_b: np.ndarray
    ) -> Tuple[Optional[int], int, str, str]:
        """
        ...
        if best_overlap > 0:
            ...
            direction_desc = (
                "A right + B left" if best_direction == "normal" else "A left + B right"
            )
            return (
                best_overlap,
                best_offset,
                f"Horizontal merge ({direction_desc}): {best_overlap}px overlap (match rate: {match_ratio:.1f}%)",
                best_direction,
            )
        else:
    ...
    def merge_images(
        ...
        if merge_type == "vertical":
            # 縦方向結合 - 両方向を自動検出
            overlap, offset, message, direction = self.find_best_overlap_vertical(
                np_a, np_b
            )
            ...
            if overlap is not None:
                ...
                mask_a, mask_b = self.create_overlap_mask(
                    np_a, np_b, merge_type, overlap, direction
                )
            ...
            # 横方向結合 - 両方向を自動検出
            overlap, offset, message, direction = self.find_best_overlap_horizontal(
                np_a, np_b
            )
            ...
            return (
                None,
                f"❌ Invalid merge type: {merge_type} (Please specify 'vertical' or 'horizontal')",
                None,
                None,
            )
